...
def tts_filter(
    ...
    if remove_special_char:
        try:
            ...
            logger.warning(f"Text: {text}")
            logger.warning("Skipping...")
    if lowercase_abbreviations:
        try:
            text = lowercase_special_abbreviations(text, lowercase_abbreviations)
        except Exception as e:
            logger.warning(f"Error transforming abbreviations to lowercase: {e}")
            logger.warning(f"Text: {text}")
            logger.warning("Skipping...")
    if translator:
        try:
...
def lowercase_special_abbreviations(text:str, abv: list = []) -> str:
    """
    changes specific abbrevaitions to lowercase for ease of pronounciation making it sound more like a word.

    Args:
        text (str): The text to filter.
        abv (list): The abbreviations to change to lowercase
    ...
    for word in abv:
        transformed_text = transformed_text.replace(word, word.lower())
